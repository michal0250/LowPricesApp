...
public class ProductService {
	...
	public void checkForProductsChangePrice(List<Product> allProducts)
		...
		ParseWebPageService parseWebPageService = new ParseWebPageService();
		...
		while (productsIterator.hasNext()) {
			...
			Double actualPrice = parseWebPageService.didPriceChanged(oldProduct
					.getUrl());

			if (oldProduct.getActualPrice() != actualPrice) {
				oldProduct.setActualPrice(actualPrice);
				updateProduct(oldProduct);
				logger.info("chceck new price: new price" + oldProduct);
			} else {
				logger.info("chceck new price: no changes ");
			}
